...
func (r *paymentmethodRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	var paymentmethods []entity.Payment
	...
	req.Rows = paymentmethods

	// Hitung total data
	if errCount := r.DB.Model(&entity.Payment{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range paymentmethods {
		paymentmethods[i].PaymentName = helpers.TruncateString(paymentmethods[i].PaymentName, 47)
	}
