...
func (r *paymentRepository) Create(req dto.PaymentRequest) (dto.PaymentResponse, error) {
...
func generatePaymentID(prefix string, autoIncrement int) (string, error) {
	...
	// Generate a secure random part of the payment ID
	securePart, err := generateSecurePart()
	if err != nil {
		return "", err
	}

	// Combine the prefix, secure random part, and auto-increment value to form the final payment ID
	return fmt.Sprintf("%s%s%s", prefix, securePart, autoIncStr), nil
}

// Function to generate a secure random part for the payment ID
...
func (r *paymentRepository) CheckMerchantDetail(merchantID string, merchantDetail *entity.MerchantDetail) error {
